...
require_once ROOT_PATH . '/php/admin_auth_check.php'; // For checking admin status
require_once ROOT_PATH . '/php/user_auth_check.php';  // For checking public user status
...
<body>
                    ...
                    <?php switch ($contentData['content_type']):
                        ...
                        case 'event': ?>
                            <?php if (!empty($contentData['banner_url'])): ?>
                                <img src="<?php echo BASE_URL . '/' . htmlspecialchars($contentData['banner_url']); ?>" alt="Event Banner" class="event-banner">
                            <?php endif; ?>
                            ...
                            <div class="gallery-grid">
                                <?php if (!empty($contentData['images'])): ?>
                                    <?php foreach ($contentData['images'] as $image): ?>
                                        <a href="<?php echo BASE_URL . '/' . htmlspecialchars($image['image_src']); ?>" target="_blank">
                                            <img src="<?php echo BASE_URL . '/' . htmlspecialchars($image['image_src']); ?>" alt="<?php echo htmlspecialchars($image['title']); ?>">
                                        </a>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </div>
                        ...
                        case 'video': ?>
                            ...
                            <?php if (!empty($contentData['uploaded_src'])): ?>
                                ...
                                <video controls style="width:100%; border-radius: 0.5rem;">
                                    <source src="<?php echo BASE_URL . '/' . htmlspecialchars($contentData['uploaded_src']); ?>" type="video/mp4">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const API_ENDPOINT = '<?php echo BASE_URL; ?>/api/interactions.php';
